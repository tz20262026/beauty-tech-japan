...
export const metadata: Metadata = {
  ...
  title: "美容ダイエット完全ガイド2026年版【食事・運動・サプリで痩せる方法】",
  description:
  ...
  return (
    <main className="bg-gray-950 min-h-screen text-gray-100">
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <Breadcrumb variant="dark" items={[{ name: "ホーム", href: "/" }, { name: "美容ダイエットガイド" }]} />
          ...
          <div className="space-y-4">
            {DIET_METHODS.map((dm, i) => (
              <div key={i} className="bg-gray-900 border border-gray-700 rounded-xl p-5">
                ...
                <p className="text-gray-300 text-xs leading-relaxed mb-3">{dm.desc}</p>
                <div>
                  <p className="text-green-400 text-xs font-bold mb-1">✅ 実践のコツ</p>
                  <ul className="space-y-0.5">
                    {dm.dos.map((d, j) => <li key={j} className="text-gray-300 text-xs">・{d}</li>)}
                  </ul>
                </div>
              </div>
            ))}

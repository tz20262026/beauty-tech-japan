...
## 🚨 エラー報告を受けた時の鉄則（絶対に守る）
- **「〇〇が壊れている」「エラーが出る」と言われたら、コードを読む前にまず全ページをHTTPチェックする**
- 思い込みで「ここが原因だ」と決めつけて直すのは禁止
- 正しい手順：
  1. 全ページを node スクリプトで一括 HTTP GET チェック
  2. 問題箇所を全部洗い出す
  3. まとめて修正する
  4. 再チェックして全ページ ✅ を確認してから完了報告
- **過去の失敗例**：フッターの404を「コードを見て部分修正」→ タグページ全130件の二重エンコードバグを見逃した。ユーザーに「なんで最初からやらない？」と強く指摘された
